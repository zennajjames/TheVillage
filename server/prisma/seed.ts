...
async function main() {
  ...
  const minneapolisSchools = [
    ...
    {
      name: 'Hale STEM School',
      description: 'Hale STEM School focuses on science, technology, engineering, and math for students in grades K-8.',
    ...
    {
      name: 'Lake Harriet Community School',
      description: 'Lake Harriet serves K-8 students with a community-focused approach to education in Southwest Minneapolis.',
  ...
  for (const school of minneapolisSchools) {
    const community = await prisma.community.upsert({
      where: {
        // Using a combination of name and zipCode as unique identifier
        id: `${school.name}-${school.zipCode}`.toLowerCase().replace(/\s+/g, '-'),
      },
      update: {},
      create: {
        name: school.name,
